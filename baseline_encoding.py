...
def declare(trace, max_lines):
    ...
    for j in range(max_lines):
        ...
        #print("l1:", Implies(LineType.is_Action(line), And(LineType.action_val(line) >= 0, LineType.action_val(line) < len(action_ids))))
        ...
        #print("l2:", Implies(LineType.is_Pred(line), And(LineType.pred_val(line) >= 0, LineType.pred_val(line) < preds)))
        ...
        for child_idx in range(2): # loop over the two children
            ...
            if j >= 1:
                ...
                #print("l3:", And(0 <= child_line, child_line < j))
        ...
        #print("l4:", Implies(LineType.is_Pred(line), And(*all_child_constraints)))
    ...
    def evaluate_lines(line_number, frame):
        ...
        for current_line_number in range(line_number, 0, -1):
            ...
            for idx in possible_pred_vals:
                true_child_line = child_fn(line, 0)  # for true child
                s.add(Implies(And(is_pred_condition, pred_val == idx, Extract(idx, idx, frame_bv) == 1),
                          line_fn(true_child_line) == line_val(true_child_line, frame_bv)))  # Use evaluated result of the true child line
                ...
                false_child_line = child_fn(line, 1)  # for false child
                s.add(Implies(And(is_pred_condition, pred_val == idx, Extract(idx, idx, frame_bv) == 0),
                          line_fn(false_child_line) == line_val(false_child_line, frame_bv)))  # Use evaluated result of the false child line
